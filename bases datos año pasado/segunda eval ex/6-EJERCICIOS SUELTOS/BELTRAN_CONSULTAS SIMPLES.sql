...
SELECT ID_FAB,ID_PRODUCTO,DESCRIPCION,(EXISTENCIAS*PRECIO) AS "VALOR__INVENTARIO"
FROM PRODUCTOS;
...
SELECT NOMBRE
FROM VENDEDORES
WHERE VENTAS<500000 AND VENTAS>100000;
...
aumentado en un 21%).*/
SELECT ID_FAB,ID_PRODUCTO,DESCRIPCION,PRECIO,(PRECIO*1.21)AS "PRODUCTO CON IVA"
FROM PRODUCTOS;
...
SELECT NUM_PEDIDO,FABRICANTE,PRODUCTO,CANTIDAD,(IMPORTE/CANTIDAD)AS "PRECIO/UNI",IMPORTE
FROM PEDIDOS;
...
/*8) Obtener la suma de las ventas de los vendedores agrupados por oficina. Llamar a la columna suma VENTAS_TOTALES*/
 SELECT OFICINA_VEND,SUM(VENTAS)AS "VENTAS TOTALES"
 FROM VENDEDORES
 GROUP BY OFICINA_VEND;
 /*OTRA FORMA*/
 SELECT OFICINA,VENTAS AS "VENTAS TOTALES"
 FROM OFICINAS;
 
/*9) Queremos saber las oficinas con un promedio de ventas de sus empleados mayor que 300.000*/
SELECT OFICINA_VEND
FROM VENDEDORES
GROUP BY OFICINA_VEND HAVING AVG(VENTAS)>300000;
...
SELECT AVG(IMPORTE)AS"IMPORTE MEDIO PEDIDOS",SUM(IMPORTE) AS "IMPORTE TOTAL",AVG(IMPORTE/CANTIDAD) AS "PRECIO MEDIO DE VENTA/UNITARIO"
FROM PEDIDOS;
...
/*DUDO ENTRE UNA Y OTRA*/
SELECT AVG(IMPORTE/CANTIDAD)AS "PRECIO MEDIO PRODUCTO"
FROM PEDIDOS
WHERE FABRICANTE='ACI';
...
SELECT OFICINA_VEND,COUNT(OFICINA_VEND)AS "NUMERO DE VENDEDORES/OFICINA"
FROM VENDEDORES
GROUP BY OFICINA_VEND;
